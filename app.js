...
loginButton.addEventListener('click', async () => {
  ...
  if (error) alert('Chyba pri prihlasovaní: ' + error.message);
  else checkUser();
});
...
signupButton.addEventListener('click', async () => {
  ...
  if (error) alert('Chyba pri registrácii: ' + error.message);
  else alert('Registrácia úspešná! Prosím, potvrďte svoj email kliknutím na odkaz, ktorý sme vám zaslali.');
});
...
const fetchResults = async () => {
  ...
  try {
    const { data: polls, error: pollsError } = await supabaseClient.from('polls').select('*').eq('is_active', false);
    if (pollsError) throw pollsError;
    ...
    const { data: votes, error: votesError } = await supabaseClient.from('votes').select('poll_id, selected_option');
    ...
    for (const poll of polls) {
      ...
      pollVotes.forEach(vote => results[vote.selected_option]++);

      let maxVotes = 0;
      for (const option in results) {
        if (results[option] > maxVotes) maxVotes = results[option];
      }

      const winners = [];
      if (maxVotes > 0) {
        for (const option in results) {
          if (results[option] === maxVotes) winners.push(option);
        }
      }
      ...
      let resultsHTML = poll.options.map(option => `
                <div class="result-item ${winners.includes(option) ? 'winner' : ''}">
                    ...
                    <span class="vote-count">${results[option]} hlasov</span>
                </div>
            `).join('');
...
const castVote = async (pollId, selectedOption, user) => {
  try {
    ...
    if (error) {
      if (error.code === '23505') alert('V tomto hlasovaní ste už hlasovali.');
      else throw error;
    } else {
